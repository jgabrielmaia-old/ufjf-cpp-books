...
string run(string name, void (*method)(Book **to_sort, int size, int copy_count, int compare_count), Book **to_sort, int size)
{
    string result;
    ...
    int copy_count = 0;
    int compare_count = 0;
    double elapsed = timer(method, to_sort, size, copy_count, compare_count);
...
double timer(void (*method)(Book **to_sort, int size, int copy_count, int compare_count), Book **to_sort, int size, int copy_count, int compare_count)
{
    struct timeval begin, end;
    trace(INFO, "Contador ativado!");
    gettimeofday(&begin, 0);

    method(to_sort, size, copy_count, compare_count);
